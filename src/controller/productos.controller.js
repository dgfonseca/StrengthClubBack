...
const getContabilidadProductos = (request,response) =>{
  pool.query("select q2.nombre,q2.codigo,q2.inventario,q2.precio,q2.preciocompra,coalesce(q3.unidadesvendidas,0) as unidadesvendidas,coalesce(round(sum(q1.ingresos),2),0) as ingresos, coalesce(round(q2.egresos,2),0)as egresos, coalesce(round(sum(q1.ingresos)-q2.egresos,0),2) as utilidad \
  from( \
