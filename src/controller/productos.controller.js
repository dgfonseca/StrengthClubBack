    ...
    if(nombre && codigo && descripcion && inventario && precio && precioCompra){
        pool.query("INSERT INTO productos(nombre,codigo,descripcion,inventario,precio,habilitado,precioCompra) VALUES($1,$2,$3,$4,$5,$6,$7)", [nombre, codigo, descripcion,inventario,precio,habilitado,precioCompra], (error, results)=>{
            if (error) {
              ...
              pool.query("INSERT INTO historico_productos(producto,inventario,precioCompra,precio,fechaInicio,fechaFin) VALUES($1,$2,$3,$4, TO_CHAR(now() at TIME ZONE 'America/Bogota', 'yyyy/mm/dd HH24:MI:SS'),null)", [codigo, inventario,precioCompra,precio], (error, results)=>{
                if (error) {
                  response.status(500)
                      .send({
                        message: error
...
try {
  ...
  if(nombre && codigo && descripcion && inventario && precio){
      if(inventarioAdicional && inventarioAdicional!==0){
        ...
        await client.query("INSERT INTO historico_productos(producto,inventario,precioCompra,precio,fechaInicio,fechaFin) VALUES($1,$2,$3,$4,TO_CHAR(NOW() AT TIME ZONE 'America/Bogota', 'yyyy/mm/dd HH24:MI:SS') ,null)", [codigo, inventarioAdicional,precioCompra,precio])
      }else{
        ...
        await client.query("UPDATE historico_productos SET fechaFin=TO_CHAR(now() at TIME ZONE 'America/Bogota', 'yyyy/mm/dd HH24:MI:SS') WHERE producto=$1 and fechaFin is null",[codigo]);
        await client.query("INSERT INTO historico_productos(producto,inventario,precioCompra,precio,fechaInicio,fechaFin) VALUES($1,$2,$3,$4, TO_CHAR(NOW() AT TIME ZONE 'America/Bogota', 'yyyy/mm/dd HH24:MI:SS'),null)", [codigo, inventario,precioCompra,precio])
      }
...
const getContabilidadProductos = (request,response) =>{
  pool.query("select q2.nombre,q2.codigo,q2.inventario,cast(q2.precio as money) as precio,cast(q2.preciocompra as money) as preciocompra,coalesce(q3.unidadesvendidas,0) as unidadesvendidas,cast(coalesce(round(sum(q1.ingresos),2),0) as money) as ingresos, cast(coalesce(round(q2.egresos,2),0) as money)as egresos, cast(coalesce(round(sum(q1.ingresos)-q2.egresos,0),2)as money) as utilidad \
  from( \
