      ...
      if(entrenadorRes.rowCount<1){
        ...
        if(countRes.rows[0].count>0){
            response.status(400).send({
              message: "Ya hay sesiones agendadas para el entrenador "+entrenador.replaceAll("%",'')+" en el horario: "+fecha,
              code: 3
          })
          return;
        }
        if(countRes2.rows[0].count>0){
            response.status(400).send({
              message: "Ya hay sesiones agendadas para el cliente"+cliente.replaceAll("%",'')+" en el horario: "+fecha,
              code: 4
          })
          return;
        }
