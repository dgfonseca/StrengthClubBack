...
const getAbonos = (request,response) =>{
  let query = "SELECT c.nombre, a.id, a.fecha, cast(a.valor as money) as valor, a.usuario, a.tipo FROM abonos inner join clientes c on c.cedula=a.cliente"
  pool.query(query,(error,results)=>{
    if (error) {
      response.status(500)
          .send({
...
const postAbono = (request, response)=>{
  ...
  if(fecha!==null && fecha!==undefined && fecha !==""){
    ...
    pool.query("INSERT INTO abonos(cliente,valor,fecha,usuario,tipo) VALUES ($1,$2,to_char(current_timestamp,'YYYY-MM-DD HH24:MI'),$3,$4)",[cliente,abono,usuario,tipo],(error,results)=>{
      if (error) {
        response.status(500)
            .send({
              message: error
